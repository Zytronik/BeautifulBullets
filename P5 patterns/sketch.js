...
function pattern11() {
  ...
  for (let i = 0; i <= pointsPos.length - 1; i++) {
    ...
    if (points == 1) {
      ...
      let maxSpeed = 0.5,
      homingForce = 2;
      trajectory[i][3] += Math.cos(angle + c / (points - 1) * trajectory[i][2]) * (maxSpeed * (homingForce + (homingForce / 3)));
      trajectory[i][4] += Math.sin(angle + c / (points - 1) * trajectory[i][2]) * (maxSpeed * homingForce) / 4;
    }
