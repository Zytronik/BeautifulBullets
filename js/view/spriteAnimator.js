...
export class SpriteAnimator {
            ...
            for (const role in allSprites[character]) {
                for (const state in allSprites[character][role]) {
                    allSprites[character][role][state]["urls"].forEach((url) => {
                        this.#loadSprite(url, loadCompleted, character, role, state);
                    });
                }
            }
        } 
    }
    #loadSprite(sprite, onComplete, character, player, state) {
        var onLoad = (e) =>  {
            ...
            this.loadedSprites[character][player][state]["urls"] = e.target;
            onComplete();
        }
        var img = new Image();
    ...
    getCurrentChallengerSprite(){
        return challenger.sprites[this.#getCurrentChallengerState()].urls;
    }
    getCurrentBossSprite(){
        return boss.sprites[this.#getCurrentBossState()].urls;
    }
    #countAllSprites(){
        let unloadedSprites = [];
        let allSprites = this.#getAllSprites();
